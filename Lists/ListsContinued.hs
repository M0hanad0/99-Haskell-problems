...
data RLEData a = Single a | Multiple Integer a deriving (Eq)

instance Show (RLEData a) where
  show a = putStrLn a
...
rotate xs 0 = xs
rotate xs n
  | n > 0 = drop (fromIntegral n) xs ++ take (fromIntegral n) xs
  | otherwise = drop len xs ++ take len xs
  where
    len = fromIntegral (length xs + fromIntegral n)
